...
export function MortalityChart({
  ...
  return (
    <Card
      className={cn(
        "p-4",
        hasAlerts && "border-red-200 bg-red-50/30 dark:bg-red-950/10",
      )}
    >
        ...
        <BarChart data={data}>
          ...
          <Bar dataKey="rate" name="Mortality Rate" radius={[4, 4, 0, 0]}>
            {data?.map((entry, i) => (
              <Cell
                key={i}
                fill={entry.rate > threshold ? "#ef4444" : "#f59e0b"}
              />
            ))}
          </Bar>
        </BarChart>
      ...
      {hasAlerts && (
        <p className="mt-2 text-xs text-red-600 text-center">
          Mortality exceeded threshold in{" "}
          {data.filter((d) => d.rate > threshold).length} of the last 4 weeks
        </p>
      )}
    </Card>
  );
