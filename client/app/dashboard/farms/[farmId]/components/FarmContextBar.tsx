    ...
    <div className="sticky top-0 z-20 -mx-6 border-b bg-background/95 px-6 py-4 backdrop-blur-sm shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
              <span className="flex items-center gap-1.5">
                ...
                {farm.region}
                {farm.subRegion && `, ${farm.subRegion}`}, {farm.country}
              </span>
              <span className="flex items-center gap-1.5">
              ...
              <span className="flex items-center gap-1.5">
                ...
                Since {formatDate(farm.createdAt)}
              </span>
              <span className="flex items-center gap-1.5">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <button
            onClick={() =>
              router.push(`/dashboard/farms/${farm.id}/records/new`)
            }
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuItem
                onClick={() =>
                  router.push(`/dashboard/farms/${farm.id}/settings`)
                }
              >
                Edit Farm Details
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => router.push(`/dashboard/farms/${farm.id}/team`)}
              >
                Manage Team Members
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() =>
                  router.push(`/dashboard/farms/${farm.id}/analytics`)
                }
              >
                View Analytics
              </DropdownMenuItem>
