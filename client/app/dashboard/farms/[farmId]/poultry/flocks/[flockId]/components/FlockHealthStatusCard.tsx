...
export function FlockHealthStatusCard({
  summary,
  performance,
  flock,
}: FlockHealthStatusCardProps) {
  // ✅ Safe access with fallbacks
  const healthStatus = summary?.summary?.healthStatus || "UNKNOWN";

  const statusConfig = {
    HEALTHY: {
      color: "bg-emerald-100 text-emerald-700 dark:bg-emerald-950/30",
      icon: CheckCircle2,
    },
    AT_RISK: {
      color: "bg-amber-100 text-amber-700 dark:bg-amber-950/30",
      icon: AlertCircle,
    },
    CRITICAL: {
      color: "bg-rose-100 text-rose-700 dark:bg-rose-950/30",
      icon: AlertCircle,
    },
  ...
  const config =
    statusConfig[healthStatus as keyof typeof statusConfig] ||
